% This code is slightly modified from the code 'cnn_stn_cluttered_mnist_init.m' in MatConvNet.

function nn = deep_thermal_imaging_init_basedonn_stncnn(imsz, use_transformer, classn, use_dout)
% script to initialize a small spatial transformer network
  ...
  % classification net:
  c_cnv1 = dagnn.Conv('size',[7 7 1 12],'pad',0,'stride',1,'hasBias',true);
  ...
  c_cnv2 = dagnn.Conv('size',[5 5 12 24],'pad',0,'stride',1,'hasBias',true);
  ...
  c_fc1 = dagnn.Conv('size',[4 4 24 48],'pad',0,'stride',1,'hasBias',true);
  nn.addLayer('c_fc1', c_fc1, {'xc6'}, {'xc7'}, {'cfcf','cfcb'});
  nn.addLayer('c_re3', dagnn.ReLU(), {'xc7'}, {'xc8'});

  % we recommend to use 'dropout' for materials with high thermal dynamics
  % (outside etc.)
  switch use_dout
     case 0      
          c_fc2 = dagnn.Conv('size',[1 1 48 classn],'pad',0,'stride',1,'hasBias',true);
     ...
     case 1
          ...
          dropoutBlock2 = dagnn.DropOut('rate',0.3);
          nn.addLayer('dropout2',dropoutBlock2,{'pred'},{'drop2'},{});

          % softmax loss:
          nn.addLayer('loss', dagnn.Loss('loss', 'softmaxlog'), {'drop2','label'}, 'objective');
          nn.addLayer('error', dagnn.Loss('loss', 'classerror'), {'drop2','label'}, 'error');                 
  end
  ...
  nn.meta.trainOpts.numEpochs = 350 ;
end
